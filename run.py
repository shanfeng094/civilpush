...
def dele(bg_id):
    if session.get('log'):
        try:
            ...
            cur.execute('DELETE FROM tag WHERE blog = %s ',(bg_id,))
            cur.execute('DELETE FROM comm WHERE blog = %s ',(bg_id,))
            get_db().commit()
        except:
            return redirect(url_for('page',pg=1))
    return redirect(url_for('page',pg=1))
...
def page(pg):
    cur=get_db().cursor()
    cur.execute(' SELECT id, title, abstract,tag,date,file FROM blog ORDER BY id DESC LIMIT 8 OFFSET %s',(pg*8-8,))
    tem=cur.fetchall()
    cur.execute('SELECT count(*) FROM blog;')
    pmax=((cur.fetchall()[0][0]+7)/8 or 1)
    if pg > pmax or pg < 1:
...
def article(bg_id):
    if request.method == 'POST':
        if request.form['comment']:
                author = request.form['author'] or u'访客'
                cur=get_db().cursor()
                cur.execute('insert into comm (content, author, blog, reply) values (%s, %s, %s, %s)', (request.form['comment'], author, bg_id, request.form['reply'] or None))
                get_db().commit()
                return redirect(url_for('article',bg_id=bg_id))
    try:
        cur=get_db().cursor()
        cur.execute('SELECT title, date, content, tag, abstract from blog where id=%s',(bg_id,))
        cont=cur.fetchall()[0]
    except:
        return render_template('error.html'), 404
    else:
        cur.execute(' SELECT content, date, author,id,reply FROM comm WHERE blog=%s ORDER BY id DESC',(bg_id,))
        tem=cur.fetchall() or []
        tem=list(tem)
...
def arch(arc,pg):
    cur=get_db().cursor()
    cur.execute(' SELECT id, title, abstract,tag,date,file FROM blog WHERE file=%s ORDER BY id DESC LIMIT 8 OFFSET %s',(arc,pg*8-8,))
    tem=cur.fetchall()
    try:
...
def tag(tag,pg):
    ...
    item=0
    for blog in blogs:
        cur.execute(' SELECT id, title, abstract,tag,date,file FROM blog WHERE id=%s',(blog,))
...
def wish():
    if request.method == 'POST':
        ...
        if request.form['comments']:
                ...
                cur=get_db().cursor()
                cur.execute('insert into comm (content, author, blog) values (%s, %s, %s)', (request.form['comments'], author, -1))
                get_db().commit()
                return render_template('wish.html')
        if request.form['commentm']:
                author = request.form['authorm'] or u'访客'
                cur=get_db().cursor()
                cur.execute('insert into comm (content, author, blog) values (%s, %s, %s)', (request.form['commentm'], author, -2))
                get_db().commit()
                return render_template('wish.html')
    return render_template('wish.html')
